...
portrait: colored key/value rows for current work, tech stack, and
...
EDIT THE ROWS BELOW to match your own bio -- this file is the one place in the
whole pipeline where you're expected to hand-tune the text.
"""
import html
import os
...
ROWS = [
    ...
    ("kv", "Backend", "Node, Firebase, Postgres/Prisma"),
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">adi@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">adi</tspan><tspan fill="{MUTED}">@</tspan>'
